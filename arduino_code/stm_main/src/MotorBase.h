...
static int quadTimer[2] = {QUAD_TIMER_2, QUAD_TIMER_3};
static int pwmPin[2] = {PB9, PB8};
static int pwmTimer[2] = {4, 4};
...
public:
    // Config
    MotorBase(int x, unsigned int T);
    void begin();
    ...
    long getCurrTicks();                // count/s
    long getPrevTicks();                // counts/s
    long getDeltaTicks();               // counts/s
};
